...
export abstract class BaseArrService {
  ...
  async historyDetail(
    ...
    try {
      ...
      const items = records.slice(0, 20).map((item: any) => ({
        id: item.id,
  ...
  async search(
    ...
    try {
      ...
      const searchResults = limitedResults.map((item: any) => ({
  ...
  async addNew(request: AddRequest): Promise<OperationResult<AddData>> {
    const operation = withMetrics(this.serviceName, "addNew", async () => {
      ...
      if (!qualityProfileId) {
        const profiles: any = await fetchJson(
        ...
        if (!qualityProfileId) {
          throw new Error(
            `Unable to auto-select quality profile for ${this.serviceName}. ` +
              `Available profiles: ${profiles.map((p: any) => `${p.name} (id: ${p.id})`).join(", ")}. ` +
              `Please specify qualityProfileId explicitly.`,
          );
        }
      }
  ...
  async importIssues(): Promise<OperationResult<ImportIssueData>> {
    ...
    try {
      ...
      const issues = records.slice(0, 10).map((item: any) => ({
        id: item.id,
        title: item.title,
  ...
  private analyzeQueueItem(item: any): QueueIssueAnalysis {
  ...
  private selectBestQualityProfile(profiles: any[]): number | null {

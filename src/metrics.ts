...
class MetricsCollector {
  ...
  recordOperation(
    ...
    serviceMetrics.averageResponseTime =
      (serviceMetrics.averageResponseTime * (serviceMetrics.totalRequests - 1) + duration) /
      serviceMetrics.totalRequests;
  ...
  getSummary(): {
    ...
    const successfulRequests = Object.values(this.metrics.services)
      .reduce((sum, s) => sum + s.successfulRequests, 0);
    const totalRequests = this.metrics.totalRequests;
    const successRate = totalRequests > 0 ? (successfulRequests / totalRequests) * 100 : 100;

    // Calculate overall average response time
    const totalDuration = Object.values(this.metrics.services)
      .reduce((sum, s) => sum + (s.averageResponseTime * s.totalRequests), 0);
    const averageResponseTime = totalRequests > 0 ? totalDuration / totalRequests : 0;

    // Count error frequencies
    const errorCounts = new Map<string, number>();
    this.metrics.recentEntries
   ...
   */
  getRecentOperations(limit: number = 20): MetricEntry[] {
  ...
  getHealthStatus(): {
    ...
    const recentFailureRate = recentEntries.length > 0 ?
      (recentFailures / recentEntries.length) * 100 : 0;
          ...
          name,
          {
            ...
            operations: Object.fromEntries(
              Object.entries(metrics.operations).map(([op, opMetrics]) => [
                ...
                {
                  ...opMetrics,
                  averageDuration: opMetrics.count > 0 ?
                    opMetrics.totalDuration / opMetrics.count : 0,
                  failureRate: opMetrics.count > 0 ?
                    (opMetrics.failures / opMetrics.count) * 100 : 0,
                },
              ])
            ),
          },
...
export function withMetrics<T extends any[], R>(
  ...
  return async (...args: T): Promise<R> => {
    ...
    try {
      ...
      metricsCollector.recordOperation(service, operation, duration, success, error);
    }
  };
}
...
export class Timer {
  ...
  record(service: string, operation: string, success: boolean, error?: string): void {

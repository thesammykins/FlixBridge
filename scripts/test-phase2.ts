...
async function loadTestConfig(): Promise<TestConfig> {
  ...
  if (!config.sabnzbd || !config.sonarr || !config.radarr) {
    try {
      const fs = await import("fs/promises");
      const configData = JSON.parse(await fs.readFile("config.json", "utf-8"));

      if (!config.sabnzbd && configData.downloaders?.sabnzbd) {
        config.sabnzbd = configData.downloaders.sabnzbd;
      }
      if (!config.sonarr && configData.services?.sonarr) {
        config.sonarr = configData.services.sonarr;
      }
      if (!config.radarr && configData.services?.radarr) {
        config.radarr = configData.services.radarr;
      }
    } catch (error) {
...
async function main() {
  ...
  if (!config.sabnzbd) {
    ...
    console.log("   Or add 'downloaders.sabnzbd' section to config.json");
    process.exit(1);
  }

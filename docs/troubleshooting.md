...
4. **Verify environment variables:**
   ...
   # Check standard fallback variables
...
2. **Update service names in config:**
   ```json
   // Fix naming to include proper keywords
   "tv-manager" → "sonarr-main"
   "movie-server" → "radarr-main"
   ```
...
1. **Use exact service names:**
   ```bash
   # Check your config.json for exact names
   cat config.json | grep -A 2 "services"
...
**Fix:** Verify config file exists and has valid JSON
...
2. **Configuration (sanitized):**
   ...
   cat config.json | sed 's/"apiKey": ".*"/"apiKey": "[REDACTED]"/g'
   ```
